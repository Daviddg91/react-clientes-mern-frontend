...
import { clientStartUpdate } from '../../actions/clients';
...
const dispatch = useDispatch();
    ...
    const cliente = useSelector(state => state.clients.clientes.find((item) => item.id === id ? item : false));
    
    const [ values, handleChange , reset ] = useForm(cliente);
    ...
    const validateRepeatDni = async(dni) => {
      if(dni.length === 9){
        if(validateErrors.errors.dni !== null){
          
              const baseUrl = process.env.REACT_APP_API_URL;
              const url = ` ${baseUrl}/clients/getClient/${dni}`;
    ...
    const handleSubmit = async(e) => {
      e.preventDefault();
        ...
        try {
         dispatch( clientStartUpdate( values ));
          setTimeout(() => {
